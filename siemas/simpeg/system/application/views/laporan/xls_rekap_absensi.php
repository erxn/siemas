...
$xls->getProperties()->setCreator("SIM Puskesmas Bogor Tengah")
        ...
        ->setTitle("Daftar Absensi Pegawai")
        ->setSubject("Daftar Absensi Pegawai Puskesmas Bogor Tengah");
...
$xls->getActiveSheet()->mergeCells("A2:{$kolom_jumlah}2")->setCellValueByColumnAndRow(0, 2, "DAFTAR ABSENSI PEGAWAI");
...
foreach ($absensi_pkm as $a) {
    ...
    for ($j = 1; $j <= $jumlah_hari_bulan_ini; $j++) {
        if (in_array($j, $tanggal_libur_pkm)) {
            ...
            if ($a['hadir_' . $j] == '1') {
                $xls->getActiveSheet()->setCellValueByColumnAndRow($j + 1, $baris, "√");
                $xls->getActiveSheet()->getStyleByColumnAndRow($j + 1, $baris)->getAlignment()->setHorizontal('center');
            }
        }
    }
    
    $xls->getActiveSheet()->setCellValue($kolom_jumlah . $baris, $a['jumlah']);
...
$xls->getActiveSheet()->getColumnDimension($kolom_jumlah)->setWidth(7);
for ($j = 1; $j <= $jumlah_hari_bulan_ini; $j++) {
